...
The core plan (`2026-06-16-laravel-ai-guardrails.md`) ships **PHP + Artisan surfaces only** for v0.1 and explicitly defers the HTTP/MCP surface (Assumption #4). **Therefore EVERY endpoint below is a "core must add this endpoint" gap** — the admin requires an HTTP read/config API the core does not yet expose. These map 1:1 onto the core's existing services (`InjectionAuditStore::recent()`, the firewall validator/scoper, the config toggles, `ApprovalRouter`, `AiGuardrails::screen()/sanitize()/validateStructured()`), so the work is thin controllers over existing code. All paths are relative to `api_base` (default `/admin/ai-guardrails/api`). All responses include `schema_version`.
