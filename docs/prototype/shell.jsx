...
function Sidebar({ route, onNavigate, counts = {} }) {
  const items = [
    { key: 'home', label: 'Overview', icon: <I.Home size={15}/> },
    { key: 'items', label: 'Items', icon: <I.Runs size={15}/>, badge: counts.items || null },
    { key: 'tasks', label: 'Tasks', icon: <I.Approvals size={15}/>, badge: counts.tasks || null },
    { key: 'events', label: 'Events', icon: <I.Outbox size={15}/>, badge: counts.events || null },
  ];
  const secondary = [
    { key: 'library', label: 'Library', icon: <I.Definitions size={15}/> },
  ...
  return (
    <header className="topbar">
      <div className="crumbs">
        ...
        <b>{(route || 'home').replace(/^./, c => c.toUpperCase())}</b>
      </div>
